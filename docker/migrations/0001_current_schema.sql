...
SELECT
  ...
  0
WHERE NOT EXISTS (
  SELECT 1 FROM security_policies
  WHERE tenant_id = '00000000-0000-0000-0000-000000000000'
    AND name = 'Default Enterprise Policy'
);
...
INSERT INTO pii_patterns (tenant_id, pattern_name, regex_pattern, replacement_strategy)
SELECT v.tenant_id::uuid, v.pattern_name, v.regex_pattern, v.replacement_strategy
FROM (VALUES
...
) AS v(tenant_id, pattern_name, regex_pattern, replacement_strategy)
WHERE NOT EXISTS (
  SELECT 1 FROM pii_patterns p
  WHERE p.tenant_id = v.tenant_id::uuid AND p.pattern_name = v.pattern_name
);
